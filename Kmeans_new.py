...
with open('Cluster_1_2.csv', newline='') as csvfile:
    ...
    Features = ['Spike Freq','QW spike freq','NREM spike freq','Burst freq','QW burst freq','NREM burst freq','Theta LFP','Delta LFP','Theta ff eeg','Delta ff eeg','Theta fp eeg','Delta fp eeg']
...
df.to_csv("CellName_cluster1_new.csv", index=False)


df = pd.DataFrame({"Cl2" : Dat_np[Cl2,0]})
df.to_csv("CellName_cluster2_new.csv", index=False)


df = pd.DataFrame({"Cl3" : Dat_np[Cl3,0]})
df.to_csv("CellName_cluster3_new.csv", index=False)
print(Dat_np[Cl1,0])
